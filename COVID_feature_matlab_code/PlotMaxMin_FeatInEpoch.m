...
%% frequency features 
% SNR br _hr_
patient_rr=opt.patient_rr;
patient_hr=opt.patient_hr;
L = length(Data);
Y = fft((Data));
P2 = abs(Y/L);
P1 = P2(1:L/2+1);
...
feature_f=[signal_rr*60,signal_hr*60,snr_hr,snr_br,...
  power_spec(1),power_spec(2),power_spec(3),power_spec(4),power_spec(5),...
  power_ratio(1),power_ratio(2),power_ratio(3),power_ratio(4),power_ratio(5)];          
